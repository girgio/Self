...
if(_selezione == "azione"){
    ...
    if(_selezione_magia){
        ...
        for(var i = 0;i < array_length(_manager.players[_manager._turn].data.items);i++){
        draw_set_font(Magic_font)
        var _y_text = global.h_res*(_y_perc_text+0.05*i)
        var _x_text = obj_battle_light._x_perc*global.w_res
        draw_text(_x_text,_y_text,_manager.players[_manager._turn].data.items[i].name)
        draw_text_colour(_x_text + 0.6*_w_box,_y_text,$"X{_manager.players[_manager._turn].data.num_items[i]}",c_blue,c_aqua,c_blue,c_aqua,1)
        draw_set_font(Font3)
        }
}
}
